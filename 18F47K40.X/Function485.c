...
//uint8_t Baptesim = 0;           //0: default, non assegnato, ï¿½ncora tirata
                                //1: assegnato, ï¿½ncora rilasciata
...
uint8_t Check485RX()
{
    ...
    switch (funct)
    {
        ...
        case COMM_FNC_SET_ADDR:
            if (MDB_addr == '0')                      //Non ï¿½ inizializzato: devo settarmi
            {
                t1 = *(data);
                t1 -= '0';
                t2 = *(data+1);
...
//                answLen += *payl_answ;               // debug only!
            	...
            	//NON deve rispondere! risponderï¿½ il successivo (se esiste) etc!!!
            	return 0;
            }
            break;
